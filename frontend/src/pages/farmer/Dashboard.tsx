...
import {
  Plus, Edit2, Trash2, IndianRupee, Package, ShoppingBag,
...
const ChatPanel = ({
  ...
  const send = () => {
    ...
    setMessages((prev) => [
      ...prev,
      { sender: myId, content, createdAt: new Date().toISOString(), pending: true },
    ]);
  ...
  const dynamicStats = [
    {
      ...
      value: `₹${statsData.totalEarnings.toLocaleString("en-IN")}`,
      icon: IndianRupee,
      change: "",
    },
    {
    ...
    },
    {
      ...
      value: String(statsData.ordersThisMonth),
      icon: ShoppingBag,
      change: "",
    },
    {
      label: "Avg. Rating",
      value: statsData.avgRating ? `${statsData.avgRating}★` : "N/A",
      icon: TrendingUp,
      change: "",
    },
  ];
  ...
  useEffect(() => {
    ...
    const handleNotification = ({
      ...
      setProductChats((prev) => {
        ...
        if (!chat) return prev;
        const pid = chat.product?._id;
        if (pid) setUnread((u) => ({ ...u, [pid]: (u[pid] || 0) + 1 }));
        return prev;
      });
